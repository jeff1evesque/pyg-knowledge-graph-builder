...
#
# THE 0.25/0.4 DEFAULT IS A HARDWARE FLOOR, NOT AN UNTUNED VALUE -- see the README
# "Cluster prerequisites for GPU runs" section and conf/spark-rapids.conf.template. On
# an integrated / unified-memory GPU the "GPU memory" IS the host's RAM, so a large pool
# starves the OS and the JVM and drives the machine into swap. Do not raise it globally
# to fix one big run.
#
