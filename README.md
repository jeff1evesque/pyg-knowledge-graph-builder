...
| **2 — linker smokes** | Each linker's `enrich()` driven end-to-end over tiny in-memory triples: one happy path + one short-circuit (foreign input for the intra-source linkers; a single detected source, and two sources with nothing linkable, for the cross-source linker). | `test_{bls,noaa,market,sec}_linker.py`, `test_cross_source_linker.py` |
| **Targeted deep** | One focused test on each module's trickiest computation (including the negative case), where a silent regression would be costly. | severity escalation (NOAA), option moneyness (market), CIK unification (SEC), temporal sequencing (BLS), state-FIPS geographic chain (cross-source) |
| **construction internals** | Value-level unit tests of the PyG construction modules — exact node IDs, edge-index contents + `(src_id, dst_id)` ordering, config filters, determinism, the node/edge feature **encoding** (a known triple lands in the layout-reserved vector slot with the expected value: class-identity/categorical multi-hots, z-score numeric normalization, edge temporal/numeric-contrast/moneyness signals, plus `_classify_relation` per category and a categorical-determinism guard), the final `build_hetero_data` assembly (feature↔node-ID alignment via encoding-independent sentinels, edge endpoints within per-type ID ranges, per-type counts), and the six metadata JSON files' content (keys, counts, type names, feature-segment structure) — asserting what the `e2e` smoke only checks structurally or for presence. Runs in the fast suite; `test_metadata_writer.py` is pure-Python (no `SparkSession`). | `test_node_mapper.py`, `test_edge_mapper.py`, `test_feature_extractor.py`, `test_edge_feature_extractor.py`, `test_constructor.py`, `test_metadata_writer.py` |
| **e2e — pipeline smoke** (`e2e` marker, manual-only) | The real `build_graph` end-to-end over tiny committed RDF fixtures: both source loaders (`.nt` and turtle-parquet) × all three modes (`full`, and the `enrichment_only` → `pyg_only` split), asserting a valid `.pt` + all six metadata JSONs and layout-consistent tensor shapes. Also a **reproducibility** check that runs the pipeline twice (each in its own process, to isolate the per-run driver heap) and asserts the two graphs, feature tensors, and metadata are identical — an integration property no unit test can see. It currently `xfail`s on the known `jolts_*` categorical non-determinism, so the guard lands now and flips green when that bug is fixed. Catches wiring / API-mismatch bugs the unit tiers can't. | `tests/e2e/test_pipeline_smoke.py` |
