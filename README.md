...
| **e2e — pipeline smoke** (`e2e` marker, manual-only) | The real `build_graph` end-to-end over tiny committed RDF fixtures: both source loaders (`.nt` and turtle-parquet) × all three modes (`full`, and the `enrichment_only` → `pyg_only` split), asserting a valid `.pt` + all six metadata JSONs and layout-consistent tensor shapes. Also a **reproducibility** check that runs the pipeline twice (each in its own process, to isolate the per-run driver heap; the two independent runs are launched **concurrently**, which roughly halves the twin-run wall-clock at ~2x peak memory — fall back to sequential on a memory-constrained runner) and asserts the two graphs, feature tensors, and metadata are identical — an integration property no unit test can see. This is split so a regression in the already-reproducible majority isn't masked by the known bug: `test_output_is_reproducible` is a **green** guard over the node/edge inventory, per-type counts, `edge_index`, non-`jolts_*` feature tensors, and metadata (byte-identical modulo `build_timestamp`; `slot_mapping.json` compared order-insensitively), so a *new* determinism regression outside `jolts_*` fails it; `test_jolts_features_reproducible` is a non-strict **`xfail`** isolating the known `jolts_*` categorical non-determinism (the drifting feature tensors and the `slot_mapping.json` entry ordering it reorders), which flips green when that bug is fixed. Catches wiring / API-mismatch bugs the unit tiers can't. | `tests/e2e/test_pipeline_smoke.py` |
