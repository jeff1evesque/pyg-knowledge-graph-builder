...
| **construction internals** | Value-level unit tests of the PyG construction modules on hand-built triples — exact node IDs, edge-index contents + `(src_id, dst_id)` ordering, config filters, and determinism — asserting what the `e2e` smoke only checks structurally. Runs in the fast suite. | `test_node_mapper.py`, `test_edge_mapper.py` |
