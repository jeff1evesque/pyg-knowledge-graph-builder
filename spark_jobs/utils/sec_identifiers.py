"""
SEC identifier repair — one identifier, spelled one way.

The SEC source spec (spark_jobs/sources/sec.py) declares
canonicalize_sec_identifiers as its ``canonicalize``, and the loader applies it
to the rows read from SEC paths. utils/canonicalization.py says why a repair
runs in the loader and is not enrichment.
