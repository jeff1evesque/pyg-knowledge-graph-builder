"""
CPI-specific enrichment logic
"""
from rdflib import Graph
from typing import Dict, List
from glue_jobs.utils.rdf_utils import CPI, BLS_ENRICHMENT, get_month_name, get_year_value
...
class CPIEnricher(DatasetEnricher):
    """CPI-specific enrichment"""

    def __init__(self, graph: Graph):
        super().__init__(graph, CPI)
        self.month_order = [
    ...
    def get_sector_keywords(self) -> Dict[str, List[str]]:
        """Extract CPI keywords from sector patterns"""
        keywords = {}
        for sector_name, pattern in BLS_SECTOR_PATTERNS.items():
            if 'cpi' in pattern['keywords']:
                keywords[sector_name] = pattern['keywords']['cpi']
        return keywords

    def get_measurement_types(self) -> Dict[str, Dict]:
        """Return CPI measurement type configurations"""
        return MEASUREMENT_TYPES.get('cpi', {})

    def link_temporal_sequences(self) -> int:
        """Link temporal sequences for CPI measurements"""
        total_links = 0
    ...
    def _link_sequences_for_measurement(self, measurement_type, category_property,
        ...
        if links_added > 0:
            logger.info(f"  CPI.{measurement_name}: {links_added} sequence links")
