"""
LAUS-specific enrichment logic
...
class LAUSEnricher(DatasetEnricher):
    ...
    Local Area Unemployment Statistics provides state and local area
    unemployment data, including:
    - Labor force levels by state
    - Employment levels by state
    - Unemployment levels and rates by state
    ...
    def _link_sequences_for_measurement(self, measurement_type, category_property,
        ...
        # Group by category (state or industry)
        by_category = {}
        for row in results:
        ...
        if links_added > 0:
            logger.info(f"  LAUS.{measurement_name}: {links_added} sequence links")
