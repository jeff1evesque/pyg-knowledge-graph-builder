"""
XIMPIM-specific enrichment logic

Import/Export Price Indexes (11 tables)
"""
from rdflib import Graph
from typing import Dict, List
from glue_jobs.utils.rdf_utils import XIMPIM, BLS_ENRICHMENT, get_month_name, get_year_value
...
class XIMPIMEnricher(DatasetEnricher):
    """XIMPIM-specific enrichment"""

    def __init__(self, graph: Graph):
        super().__init__(graph, XIMPIM)
        self.month_order = [
    ...
    def get_sector_keywords(self) -> Dict[str, List[str]]:
        """Extract XIMPIM keywords from sector patterns"""
        keywords = {}
        for sector_name, pattern in BLS_SECTOR_PATTERNS.items():
            if 'ximpim' in pattern['keywords']:
                keywords[sector_name] = pattern['keywords']['ximpim']
        return keywords

    def get_measurement_types(self) -> Dict[str, Dict]:
        """Return XIMPIM measurement type configurations"""
        return MEASUREMENT_TYPES.get('ximpim', {})

    def link_temporal_sequences(self) -> int:
        """Link temporal sequences for XIMPIM measurements"""
        total_links = 0
    ...
    def _link_sequences_for_measurement(self, measurement_type, category_property,
        ...
        if links_added > 0:
            logger.info(f"  XIMPIM.{measurement_name}: {links_added} sequence links")
