"""
EMPSIT-specific enrichment logic
"""
from rdflib import Graph
from typing import Dict, List
from glue_jobs.utils.rdf_utils import EMPSIT, BLS_ENRICHMENT, get_month_name, get_year_value
...
class EMPSITEnricher(DatasetEnricher):
    """EMPSIT-specific enrichment"""

    def __init__(self, graph: Graph):
        super().__init__(graph, EMPSIT)
        self.month_order = [
    ...
    def get_sector_keywords(self) -> Dict[str, List[str]]:
        """Extract EMPSIT keywords from sector patterns"""
        keywords = {}
        for sector_name, pattern in BLS_SECTOR_PATTERNS.items():
            if 'empsit' in pattern['keywords']:
                keywords[sector_name] = pattern['keywords']['empsit']
        return keywords

    def get_measurement_types(self) -> Dict[str, Dict]:
        """Return EMPSIT measurement type configurations"""
        return MEASUREMENT_TYPES.get('empsit', {})

    def link_temporal_sequences(self) -> int:
        """Link temporal sequences for EMPSIT measurements"""
        total_links = 0
    ...
    def _link_sequences_for_measurement(self, measurement_type, category_property,
        ...
        if links_added > 0:
            logger.info(f"  EMPSIT.{measurement_name}: {links_added} sequence links")
