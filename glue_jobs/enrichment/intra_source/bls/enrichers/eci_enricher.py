"""
ECI-specific enrichment logic
"""
from rdflib import Graph
from typing import Dict, List
from glue_jobs.utils.rdf_utils import ECI, BLS_ENRICHMENT, get_month_name, get_year_value
...
class ECIEnricher(DatasetEnricher):
    """ECI-specific enrichment"""

    def __init__(self, graph: Graph):
        super().__init__(graph, ECI)
        self.month_order = [
    ...
    def get_sector_keywords(self) -> Dict[str, List[str]]:
        """Extract ECI keywords from sector patterns"""
        keywords = {}
        for sector_name, pattern in BLS_SECTOR_PATTERNS.items():
            if 'eci' in pattern['keywords']:
                keywords[sector_name] = pattern['keywords']['eci']
        return keywords

    def get_measurement_types(self) -> Dict[str, Dict]:
        """Return ECI measurement type configurations"""
        return MEASUREMENT_TYPES.get('eci', {})

    def link_temporal_sequences(self) -> int:
        """Link temporal sequences for ECI measurements"""
        total_links = 0
    ...
    def _link_sequences_for_measurement(self, measurement_type, category_property,
        ...
        if links_added > 0:
            logger.info(f"  ECI.{measurement_name}: {links_added} sequence links")
