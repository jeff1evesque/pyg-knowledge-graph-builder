...
The tickers CSV is produced by the schwab-morning pipeline and stored at:
...
def load_sector_patterns_from_s3(
    ...
    Reads the CSV produced by the schwab-morning pipeline's
    fetch_tickers handler (same file as latest.json). Groups tickers
    by GICS Sector column to build sector patterns.

    Args:
        bucket: S3 bucket name (e.g., "stock-market.jefflevesque.com")
        key: S3 object key (e.g., "schwab/sp500/tickers/latest.json")
