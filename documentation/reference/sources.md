...
its own body. `rdf_utils.py` imports the registry, so a Spark import at the top of
a spec module would reach every module that imports `rdf_utils.py`.

## How a run picks its sources

A run picks its sources by the paths it is given. There is no `--sources` flag.
Each path in `--source_paths` must belong to exactly one registered source:

1. **By fragment.** A path holding a source's fragment is that source's:
   `source=bls`, `source=sec`, `quotes` for market, `/noaa/` for NOAA.
2. **By name.** When no fragment matches, a folder or file named after a source
   does, such as `turtle_parquet/bls/cpi` or `ntriples/sec.nt`. Whole names only:
...
leaves out SEC keeps market's and NOAA's slots where they are. `pyg_only` reads no
source paths and picks nothing.

> **Note:** Raw RDF is produced upstream by RML mappers, which are not part of
> this repository. This pipeline assumes the RDF is already in storage, as
> N-Triples files or as Turtle in Parquet, conforming to 100+ domain-specific
> ontologies.
